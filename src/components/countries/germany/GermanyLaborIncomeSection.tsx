...
} from '../../../lib/countries/germany/germanyGovernmentPolitics';
import {
...
import type { GermanyIncomeGroupRow } from '../../../lib/countries/germany/germanyIncomeDistribution';

const GOV_CSV_URL = '/data/germany_government_politics.csv';
const LABOR_STATS_CSV_URL = '/data/germany_labor_statistics.csv';
const LABOR_INCOME_ERROR = 'Failed to load labor & income data.';
...
export const GermanyLaborIncomeSection = memo(function GermanyLaborIncomeSection({
  ...
  const { text: govRaw, error: govError } = useCsvText(
    govCsvUrl,
    isGermany ? germanyGovernmentCsvRaw : '',
    LABOR_INCOME_ERROR,
  );
  const { text: laborRaw, error: laborError } = useCsvText(
    laborCsvUrl,
    isGermany ? germanyLaborStatsCsvRaw : '',
    LABOR_INCOME_ERROR,
  );
  const loadError = govError ?? laborError;
