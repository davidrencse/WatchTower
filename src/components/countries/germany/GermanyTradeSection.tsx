...
export type TradeSectionData = {
  ...
  /** Legend and tooltip label for the comparison bars. */
  ftaSeriesLabel?: string;
  notes: ReactNode;
};
...
export const GermanyTradeSection = memo(function GermanyTradeSection({
  ...
  ftaSeriesLabel = 'Net annual GDP impact (B€)',
  notes = GERMANY_NOTES,
}: Partial<TradeSectionData> = {}) {
  const agreementChartConfig = {
    netBenefit: { label: ftaSeriesLabel, color: '#22c55e' },
  } satisfies ChartConfig;
    ...
    <div className="flex flex-col gap-4">
      ...
      <section id="trade-block-agreements" className="scroll-mt-28 space-y-4">
        ...
        <Card className="border-line bg-surface-metric">
          ...
          <CardContent className="space-y-2 p-3 pt-0">
            <ChartContainer config={agreementChartConfig} className="h-[320px] w-full font-sans">
              <ResponsiveContainer width="100%" height="100%" initialDimension={{ width: 320, height: 240 }}>
                <BarChart data={[...ftaImpacts]} margin={{ top: 8, right: 10, left: 2, bottom: 48 }}>
                  ...
                  />
                  <Bar dataKey="netBenefit" name={agreementChartConfig.netBenefit.label} radius={[6, 6, 0, 0]} isAnimationActive={false}>
                    {ftaImpacts.map((row) => (
