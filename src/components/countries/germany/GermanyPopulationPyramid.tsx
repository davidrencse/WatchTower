...
export const GermanyPopulationPyramid = memo(function GermanyPopulationPyramid({
  ...
  // No csvUrl means the caller supplied the table inline (or it is Germany's bundled copy),
  // so there is nothing to fetch — useCsvText just echoes the fallback back.
  const { text: raw } = useCsvText(
    csvUrl ?? '',
    isGermany ? germanyPopulationByAgeCsvRaw : rawCsv ?? '',
  );
