...
const MIGRANT_CRIME_ERROR = 'Failed to load migrant crime data.';
...
export const GermanyMigrantCrimeSection = memo(function GermanyMigrantCrimeSection({
  ...
  const { text: raw, error: rawError } = useCsvText(
    csvUrl,
    isGermany ? germanyMigrantCrimeRaw : '',
    MIGRANT_CRIME_ERROR,
  );
  const { text: additionalRaw, error: additionalError } = useCsvText(
    additionalCsvUrl,
    isGermany ? germanyMigrantCrimeAdditionalRaw : '',
    MIGRANT_CRIME_ERROR,
  );
  const loadError = rawError ?? additionalError;
