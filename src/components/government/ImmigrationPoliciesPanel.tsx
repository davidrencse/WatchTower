import type { ImmigrationPolicyArea, ImmigrationPolicyContext } from '../../data/government/immigrationPolicies';
import { Badge } from '../ui/badge';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '../ui/card';
import { Separator } from '../ui/separator';

/**
 * Government → Policies → Immigration Policies panel.
 *
 * Presentation extracted from the original Germany-only section so France (and
 * any later country) renders the identical context header and area-card grid
 * from its own data.
 */
...
type Props = {
  context: ImmigrationPolicyContext;
  areas: readonly ImmigrationPolicyArea[];
};

export function ImmigrationPoliciesPanel({ context, areas }: Props) {
  return (
      ...
      <Card className="border-line bg-surface-metric p-0 shadow-card ring-1 ring-white/[0.04]">
        <CardHeader className="space-y-1.5 p-3 pb-2">
          <div className="flex flex-wrap items-center gap-1.5">
            <CardTitle className={`font-sans text-xs text-neutral-100 ${UC}`}>{context.headline}</CardTitle>
            <Badge
              variant="outline"
              className="h-5 border-white/10 px-1.5 font-sans text-[9px] font-normal text-neutral-400"
            >
              {context.period}
            </Badge>
          </div>
        ...
        <CardContent className="px-3 pb-3 pt-0">
          <p className="font-sans text-[11px] leading-snug text-neutral-300">{context.summary}</p>
        </CardContent>
      </Card>
