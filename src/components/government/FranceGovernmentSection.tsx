...
import { FranceCitizenshipBlocks } from './FranceCitizenshipBlocks';
import {
  FRANCE_IMMIGRATION_POLICIES_SUBSECTION_COUNT,
  FRANCE_IMMIGRATION_POLICY_AREAS,
  FRANCE_IMMIGRATION_POLICY_CONTEXT,
} from '../../data/government/franceImmigrationPolicies';
import { FRANCE_POLICY_INFOGRAPHICS } from '../../data/government/francePolicyCards';
import {
  ASSEMBLEE_NATIONALE,
  ELECTION_NOTE,
  FRANCE_DEMOCRATIC_SENTIMENT,
  FRANCE_GOV_OVERVIEW,
  FRANCE_TRUST,
  LEGISLATIVE_2024,
  LEGISLATIVE_2024_SEATS,
  SENAT,
  TRUST_SOURCE,
  type Chamber,
  type GovStat,
} from '../../data/government/france';

/**
 * France — Government section.
 *
 * Rendered instead of `GermanyGovernmentSection` for FRA. All values come from
 * `data/government/france.ts` (published French sources), and every chart here
 * is hand-drawn SVG/CSS rather than recharts, so the section adds nothing to the
 * country-page chart bundle.
 */
      ...
      <CardContent className="p-4 sm:p-5">
        ...
        {chamber.majorityThreshold != null ? (
          <p className="mt-1 text-center font-sans text-[10px] text-neutral-500">
            Dashed line = {chamber.majorityThreshold}-seat absolute majority
          </p>
        ) : null}
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between gap-2">
        ...
        <span className="font-sans text-[10px] font-semibold tabular-nums text-neutral-200">{value.toFixed(2)}%</span>
      </div>
...
function ElectionBlock() {
  const max = 40;
  const totalSeats = LEGISLATIVE_2024_SEATS.reduce((n, s) => n + s.seats, 0);
  return (
    <Card className="border-line bg-surface-metric shadow-card">
      <CardHeader className="space-y-1 p-4 pb-2">
        ...
        <CardDescription className={`font-sans text-[10px] text-neutral-500 ${UC_META}`}>
          Snap election called after the dissolution of 9 June 2024
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4 p-4 pt-2">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {LEGISLATIVE_2024.map((round) => (
            <div key={round.label} className="space-y-2.5 rounded-md border border-white/[0.06] bg-white/[0.02] p-3">
              <div className="flex items-baseline justify-between gap-2">
                <p className={LABEL}>{round.label}</p>
              </div>
              {round.shares.map((s) => (
                <ShareBar key={s.label} label={s.label} value={s.value} color={s.color} max={max} />
              ))}
              <p className="pt-1 font-sans text-[10px] text-neutral-500">
                Turnout <span className="tabular-nums text-neutral-300">{round.turnout}%</span>
              </p>
            </div>
          ))}
        </div>
...
function TrustRow({ label, value, delta }: { label: string; value: number; delta?: number }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="w-[42%] shrink-0 truncate font-sans text-[11px] text-neutral-300">{label}</span>
...
function TrustBlock() {
  const institutions = FRANCE_TRUST.filter((t) => t.group === 'institutions');
  const political = FRANCE_TRUST.filter((t) => t.group === 'political');
  return (
    <Card className="border-line bg-surface-metric shadow-card">
      <CardHeader className="space-y-1 p-4 pb-2">
        <CardTitle className={`font-sans text-base font-semibold tracking-tight text-white ${UC_TITLE}`}>
        ...
        <CardDescription className={`font-sans text-[10px] text-neutral-500 ${UC_META}`}>
          Share of respondents who trust each institution — CEVIPOF wave 17, February 2026
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4 p-4 pt-2">
        <div className="space-y-2">
          <p className={LABEL}>Services & local government</p>
          {institutions.map((t) => (
        ...
        <div className="grid grid-cols-2 gap-2 border-t border-white/[0.06] pt-3 lg:grid-cols-3">
          {FRANCE_DEMOCRATIC_SENTIMENT.map((s) => (
...
export const FranceGovernmentSection = memo(function FranceGovernmentSection({
  collapseSignal,
  expandSignal,
  headerControls,
}: {
  ...
  const policiesCount = FRANCE_POLICY_INFOGRAPHICS.length + FRANCE_IMMIGRATION_POLICIES_SUBSECTION_COUNT;
  const citizenshipCount = 4;

  return (
    <CollapsibleFlagSection
      title="Government"
      count={FRANCE_GOV_OVERVIEW.length + parliamentCount + policiesCount + citizenshipCount}
      defaultOpen
    ...
    >
      <div className="flex flex-col gap-4">
        ...
        >
          <div className="flex flex-col gap-3">
            <div className="grid grid-cols-1 gap-3 xl:grid-cols-2">
              <ChamberHemicycle chamber={ASSEMBLEE_NATIONALE} />
              <ChamberHemicycle chamber={SENAT} />
            </div>
            <ElectionBlock />
            <TrustBlock />
          </div>
        ...
        >
          <div className="flex flex-col gap-3">
            <div className={GOV_POLICIES_GRID}>
              <PolicyCarousel cards={FRANCE_POLICY_INFOGRAPHICS} label="French policy changes by sector" />
            </div>
            <CollapsibleFlagSection
              title="Immigration Policies"
              count={FRANCE_IMMIGRATION_POLICIES_SUBSECTION_COUNT}
              defaultOpen
              uppercaseTitle
              collapseSignal={collapseSignal}
              expandSignal={expandSignal}
            >
              <ImmigrationPoliciesPanel
                context={FRANCE_IMMIGRATION_POLICY_CONTEXT}
                areas={FRANCE_IMMIGRATION_POLICY_AREAS}
              />
            </CollapsibleFlagSection>
          </div>
        </CollapsibleFlagSection>
        ...
        <CollapsibleFlagSection
          ...
          expandSignal={expandSignal}
        >
          <FranceCitizenshipBlocks />
        </CollapsibleFlagSection>

        <p className={`font-sans text-[10px] leading-relaxed text-neutral-600 ${UC_META}`}>
          All values are published figures from French institutional sources (Assemblée nationale, Sénat, INSEE,
          CEVIPOF, Ministère de l’Intérieur, Transparency International). This section does not use the generated
          fra_government_politics.csv, whose government rows were modelled estimates.
        </p>
      </div>
    </CollapsibleFlagSection>
  );
